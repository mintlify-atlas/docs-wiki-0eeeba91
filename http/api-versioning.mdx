...
`Volo.Abp.ApiVersioning.Abstractions` is the smallest module in the framework — one interface, one null implementation, one module class. Its job is to give *non-MVC* layers (the client proxy, the audit log, the local event bus, custom telemetry) a way to ask "what API version is currently being served?" without taking a hard dependency on `Microsoft.AspNetCore.Mvc.Versioning`. The MVC versioning module (when present) replaces the null implementation with a request-scoped one that reads the version from the route / header / query string; without it, callers see `null` and proceed unversioned.
...
A single DI registration. The downstream `Volo.Abp.AspNetCore.Mvc.Versioning` package replaces this singleton with a scoped implementation that reads the active version from the current `HttpContext`.
...
## Example: replacing the null implementation
    ...
    : IRequestedApiVersion, ITransientDependency
{
    ...
    public HttpContextRequestedApiVersion(IHttpContextAccessor accessor) => _accessor = accessor;

    public string? Current
    {
        get
        {
            var feature = _accessor.HttpContext?.Features
                .Get<Asp.Versioning.IApiVersioningFeature>();
...
Register it with `[Dependency(ReplaceServices = true)]` (or `context.Services.Replace(...)`) and every existing caller — client proxies, audit log, custom code — instantly reads the right value.
...
The interface is registered as a **singleton** in the abstractions module. When the full versioning package is loaded (`Volo.Abp.AspNetCore.Mvc.Versioning`), the singleton is replaced with a **scoped** implementation that depends on `IHttpContextAccessor`. Two consequences:

- A `IRequestedApiVersion` resolved from a singleton consumer always returns the null instance unless you switch your consumer to scoped too.
- Inside a request scope, the resolved value reflects the current `HttpContext` — and is cached per request because the implementation reads `HttpContext.Features` (immutable for the duration of the pipeline).
