...
- `.github/workflows/*.yml` — nine CI workflows.
...
`ls .github/workflows/` ships nine YAML files. They split into two buckets:

<CardGroup cols={2}>
  <Card title="Verification (run on PRs)" icon="vial">
    `build-and-test.yml`, `codeql-analysis.yml`, `labeler.yml`, `cancel-workflow.yml`, `angular.yml`.
  </Card>
  <Card title="Release & maintenance" icon="rocket">
    `publish-release.yml`, `update-versions.yml`, `auto-pr.yml`, `image-compression.yml`.
  </Card>
</CardGroup>
