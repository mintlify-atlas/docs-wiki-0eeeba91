...
| Recovery model | `AbpAsyncTimer` re-declares every 5 s (SDK auto-recovery disabled) | `AbpAsyncTimer` boots once; long-running `Task` then polls in `while (true)` | SDK `ServiceBusProcessor` event loop |
...
| Kafka | Topic (single partition, RF=1 by default) | `KafkaMessageConsumer.CreateTopicAsync` runs on the first timer tick. `AbpKafkaOptions.ConfigureTopic` lets you override the `TopicSpecification`; `TopicAlreadyExists` is swallowed for idempotency. Producer-side creation is left to the broker / your infra. |
