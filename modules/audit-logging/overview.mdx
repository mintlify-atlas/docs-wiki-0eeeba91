...
The `[DependsOn(typeof(AbpAuditingModule))]` line is what allows the module's `AuditingStore : IAuditingStore, ITransientDependency` to *replace* the framework's default `SimpleLogAuditingStore` (registered as `ISingletonDependency`). Both are valid registrations of the same service type; because the module-level registration runs after the framework defaults, the last-wins DI convention causes `IAuditingStore` to resolve to the database-backed store at runtime.
...
Because everything goes through `IAuditingStore`, swapping persistence is a one-liner. A common case is to ship audit logs to an OLAP system instead of (or in addition to) the relational schema:
...
`AuditingManager` resolves a single `IAuditingStore` — the last registration wins. If you need fan-out (e.g. persist to SQL **and** publish to Kafka), wrap both behind one composite implementation:

```csharp
public class CompositeAuditingStore : IAuditingStore, ITransientDependency
{
    private readonly AuditingStore _sql;
    private readonly MyKafkaAuditingStore _kafka;

    public CompositeAuditingStore(AuditingStore sql, MyKafkaAuditingStore kafka)
    {
        _sql = sql;
        _kafka = kafka;
    }
    ...
    {
        await _sql.SaveAsync(auditInfo);
        await _kafka.SaveAsync(auditInfo);
    }
}
```

and replace the `IAuditingStore` registration with `CompositeAuditingStore`.
...
- **Management UIs in the ABP commercial stack** that mount these repositories behind REST endpoints for an audit-log explorer (out of scope for this open-source documentation set — what's documented here is the domain / persistence surface every consumer uses).
