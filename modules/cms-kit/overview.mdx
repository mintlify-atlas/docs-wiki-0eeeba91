...
- **CmsUser** — `Volo.CmsKit.Users.CmsUser` is a lightweight projection of the identity user, populated by `CmsUserSynchronizer` listening to `EntityCreatedEto<IdentityUser>` / `EntityUpdatedEto<IdentityUser>` events. CMS Kit deliberately does *not* take a hard dependency on the [Identity module](/modules/identity/overview).
