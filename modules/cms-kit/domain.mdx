...
`MenuItem` carries an optional `PageId` and `RequiredPermissionName`. `PageChangedHandler` is a `ILocalEventHandler<EntityUpdatedEventData<Page>>` that keeps `MenuItem.Url` in sync with `Page.Slug`. See [Menus & Media](/modules/cms-kit/menus-and-media).
...
`CmsUser` is a *projection*, not a foreign key. `CmsUserSynchronizer` listens for distributed `EntityUpdatedEto<UserEto>` events (where `UserEto` is `Volo.Abp.Users.UserEto`, raised by Identity and any other user store) and upserts a `CmsUser` row. This lets CMS Kit avoid a hard dependency on the [Identity module](/modules/identity/overview) — a host using a different user store can publish equivalent events.
...
A trivial base class so all CMS Kit domain services share an inheritance root. Three managers (`MenuItemManager`, `MarkedItemManager`, `ReactionManager`) use it directly today; the rest (`BlogManager`, `BlogPostManager`, `BlogFeatureManager`, `CommentManager`, `GlobalResourceManager`, `MediaDescriptorManager`, `PageManager`, `RatingManager`, `TagManager`, `EntityTagManager`) still derive from `Volo.Abp.Domain.Services.DomainService` for historical reasons. New domain services should prefer `CmsKitDomainServiceBase`.
...
- `AbpDistributedEventBusModule` — for `EntityUpdatedEto<UserEto>` subscription in `CmsUserSynchronizer`.
