...
This enum is in `Volo.CmsKit.Domain.Shared` and is marked `[Flags]` — repositories can bitwise-or it (e.g. `Approved | Waiting`) when building moderation queries. It maps to a `WHERE`:
