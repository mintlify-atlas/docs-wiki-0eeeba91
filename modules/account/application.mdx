...
- **`AbpIdentityApplicationModule`** — transitively pulls in `AbpIdentityDomainModule`, giving access to `IdentityUserManager`, `IIdentityRoleRepository`, `IdentitySecurityLogManager`, and the identity event handlers, plus the Identity application services and DTOs (`IdentityUserDto`, `IdentityAppServiceBase`).
- **`AbpEmailingModule`** — gives access to `IEmailSender` and the `StandardEmailTemplates` machinery used by `AccountEmailer`.
- **`AbpUiNavigationModule`** — required because `AccountUrlNames` plugs into `AppUrlOptions` (the URL provider used to build the password-reset link inside emails).

The `AppUrlOptions` registration above is what tells `AppUrlProviderAccountExtensions.GetPasswordResetUrlAsync(...)` that the canonical reset path on the `"MVC"` application is `Account/ResetPassword`; override or add applications here to send reset emails that point back to a SPA host or a custom URL.
