...
This is a deliberate choice to avoid having to invent a one-field DTO.

<Warning>
A raw connection string in a query string can end up in HTTP-server access logs (IIS, nginx, Application Insights, reverse-proxy logs) and in ABP's own audit-log `Url` column. If your operations team retains those logs, override the controller to accept the value from the request body, or store a key here and resolve the real connection string from a secrets store.
</Warning>
