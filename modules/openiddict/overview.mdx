...
| `Volo.Abp.OpenIddict.AspNetCore` | `Volo.Abp.OpenIddict.AspNetCore/` | `AbpOpenIddictAspNetCoreModule` that calls `AddOpenIddict().AddServer(...)`, the controllers (`TokenController`, `AuthorizeController`, `UserInfoController`, `EndSessionController`), the claim‑destination pipeline, wildcard‑domain handlers, and extension grant types. |
...
flowchart TB
    ...
    subgraph AspNet["AbpOpenIddictAspNetCoreModule"]
        ...
        END[EndSessionController<br/>/connect/endsession]
...
| `POST /connect/endsession` | `EndSessionController` | RP‑initiated logout. |
