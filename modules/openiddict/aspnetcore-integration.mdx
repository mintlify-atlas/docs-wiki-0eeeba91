...
`Volo.Abp.OpenIddict.AspNetCore` is the package that **mounts the authorization server on HTTP**. It depends on the domain layer (which already replaced OpenIddict's managers, stores, and caches — see [domain](/modules/openiddict/domain)), then adds the ASP.NET Core server feature: token / authorize / userinfo / endsession controllers, claim‑destination handlers, wildcard‑domain redirect URI handlers, validation middleware, and the `AbpOpenIddictAspNetCoreOptions` knobs you use to tune the whole thing.
