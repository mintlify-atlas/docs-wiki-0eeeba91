...
- **`AbpClaimsPrincipalFactoryOptions.IsRemoteRefreshEnabled`** — turning this on enables dynamic claim refresh, which removes the need to log out / in after a role change but adds per-request HTTP traffic.
- **The `Configuration:` section in `appsettings.json`** — `ConfigurationSettingValueProvider` reads from this prefix; values you check into the repo are visible to anyone who can clone the repo.
- **The `AbpPermissionOptions.IsDynamicPermissionStoreEnabled` flag** — only safe to enable once the [Permission Management](/modules/permission-management) module is loaded.
