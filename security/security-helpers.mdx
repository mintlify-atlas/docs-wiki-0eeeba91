...
This is the options-bag the security layer hands to the permission management module. `IsDynamicPermissionStoreEnabled` flips on the database-backed `IDynamicPermissionDefinitionStore`. The default value-providers are added by `AbpAuthorizationModule` — see [Authorization](/security/authorization).
