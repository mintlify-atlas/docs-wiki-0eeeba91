...
## Channel switchers {#channel-switchers}
...
## `abp update` — `UpdateCommand` {#updatecommand}
...
## `abp suite` — `SuiteCommand` {#suitecommand}
...
## `abp prompt` — `PromptCommand` {#promptcommand}

The command itself is a no-op placeholder. The actual REPL loop lives in `CliService.RunPromptAsync` (see [architecture](/cli/architecture#cliservicerunasync--the-entry-method)). `PromptCommand` exists solely so `abp prompt` parses into a known command name (otherwise the dispatcher would fall back to `HelpCommand`).
...
## `abp help` — `HelpCommand` {#helpcommand}
