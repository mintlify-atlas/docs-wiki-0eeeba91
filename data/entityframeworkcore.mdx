...
{
    public static bool SoftDeleteFilter(bool isDeleted, bool boolParam) => throw new NotImplementedException();
    public static bool MultiTenantFilter(Guid tenantId, Guid currentTenantId, bool boolParam) => throw new NotImplementedException();
}
```

These are dispatched to the DB as functions. The benefit: EF Core's compiled query cache reuses the same plan instead of generating a new one per filter‑state combination. Toggled per provider through `AbpEfCoreGlobalFilterOptions.UseDbFunction = true` — see [`/data/efcore-sqlserver`](/data/efcore-sqlserver).
