...
## `FeatureService` and `SettingsService`

Both follow the same shape as `PermissionService` — they slice their key out of the `ApplicationConfigurationDto`:

- `FeatureService.getFeature(name)` / `getFeature$(name)` — reads `configState.getOne('features')`.
- `SettingsService.get(name)` / `get$(name)` — reads from `configState.getOne('setting')`.

A typical guard implementation reads features and settings in the same expression as permissions.
...
```ts core/src/lib/core.module.ts
export function withOptions(options: ABP.Root): EnvironmentProviders {
  return makeEnvironmentProviders([{ provide: CORE_OPTIONS, useValue: options }]);
}

export function provideAbpCore(...features: EnvironmentProviders[]) {
  return makeEnvironmentProviders([
    provideAppInitializer(() => {
      const environment = inject<ABP.Root>(CORE_OPTIONS).environment;
      const envService = inject(EnvironmentService);
      envService.setState(environment);
      return inject(ConfigStateService).refreshAppState();
    }),
    ApiInterceptor,
...
The initialiser sequence — set environment, fetch config — means routes don't activate until the policy/feature/setting store is populated, so guards never see an empty state.
