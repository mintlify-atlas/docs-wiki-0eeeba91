...
description: "A landing page for the ABP test-infrastructure: AbpIntegratedTest, AbpAspNetCoreIntegratedTestBase, per-module test bases, the framework test projects under framework/test/, and the DistEvents manual samples."
---
...
<CardGroup cols={2}>
  <Card title="framework/test/" icon="flask" href="/testing/framework-test-suite">
    78 test projects + 6 support directories covering the framework itself — `AbpTestBase` (shared helpers), `Volo.Abp.Core.Tests`, `Volo.Abp.Autofac.Tests`, `Volo.Abp.AspNetCore.*.Tests`, `Volo.Abp.EventBus.Tests`, `Volo.Abp.Caching.Tests`, and so on.
  </Card>
...
└── test/                                       // see /testing/framework-test-suite
    ...
    └── Volo.Abp.{Name}.Tests/                  // 78 individual test projects
...
4. `framework/test/` is dozens of instances of pattern 1 against in-memory backing stores; `test/DistEvents/` is a small set of console apps that test the same code against real brokers.
