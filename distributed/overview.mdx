...
Yes. The factory adds the `__tenant` header (the constant comes from `TenantResolverConsts.DefaultTenantKey`) whenever `ICurrentTenant.Id` has a value. The peer's tenant resolver picks it up like any other inbound request. No additional configuration is required for typed proxies either — `ClientProxyBase` writes the tenant header onto every `HttpRequestMessage` before it enters the `DelegatingHandler` chain, so `AbpInvocationHandler` only has to rewrite the URL.
</Accordion>
