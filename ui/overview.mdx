...
2. **Write the Razor Page.** Drop `Pages/Books/Index.cshtml(.cs)` into your module. Set `Layout = ThemingConsts.LayoutTypes.Application;` so `IThemeManager.CurrentTheme.GetLayout(...)` resolves it to `~/Themes/Basic/Layouts/Application.cshtml` (see [/ui/basic-theme](/ui/basic-theme)).
